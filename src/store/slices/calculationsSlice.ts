...
export const updateResistorValue = createAsyncThunk<void, object, AsyncThunkConfig>(
    ...
    async function({resistor_id, voltage},thunkAPI) {
        const state = thunkAPI.getState()
        await api.calculations.calculationsUpdateResistorUpdate(state.calculations.calculation.id, resistor_id, {voltage})
    }
)
